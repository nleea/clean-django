...
def calculate_exchanged_amount(
    exchange_rate: CurrencyExchangeRateEntity, amount: float
) -> CurrencyExchangeAmountEntity:
...
def filter_currencies(code: str, currencies: list) -> CurrencyEntity:
    currency = list(
        filter(lambda x: x.code == code if hasattr(x, "code") else False, currencies)
    )
